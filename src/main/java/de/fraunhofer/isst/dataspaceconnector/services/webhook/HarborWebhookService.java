...
public class HarborWebhookService extends BaseEntityService<Event, EventDesc> {
    ...
    private void pushArtifactEventHandler(Event event) throws WebhookException {
        ...
        if(!artifactService.doesExist(ids.get("appId"))){
            var app = new AppData();
            app.setAvailable(true);
            app.setNamespace(repoNamespace);
            app.setRepoFullName(repoFullName);
            app.setRepoName(repoName);
            app.setRepoType(repoType);


            ArtifactDesc artifactDesc = new ArtifactDesc();
            artifactDesc.setValue(app);

            artifactService.update(ids.get("appId"), artifactDesc);
            ...
            ArtifactDesc arifactDesc = new ArtifactDesc();
            arifactDesc.setValue(app);
            artifactService.create(arifactDesc);
        }
    }
